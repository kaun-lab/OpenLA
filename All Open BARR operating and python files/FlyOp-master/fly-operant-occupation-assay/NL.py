...
import cv2
import roi  # where is documentation for this???

import cProfile
profiler = cProfile.Profile()  # init. profile class for Python program statistics

# tkinter library name has changed from Python 2.7 to Python 3.0 and above
...
def get_elapsed_time(start_time):
    """
    Given a starting time, return how much time has passed since the starting 
    time.
    
    Parameters
    ----------
    start_time : str
        Time in the format of "%Y-%m-%d %H.%M.%S"?
    """

    return time.clock()-start_time  # current time minus start time
...
def correct_distortion(input_frame, calib_mtx, calib_dist, crop_frame=False):
    ...
    Parameters
    ----------
    input_frame : cv2 OutputArray image
        Some frames read in from the camera.
    calib_mtx : ndarray object
        Camera calibration data from the camera calibration file.
    calib_dist : ndarray object
        Camera distance data from the camera calibration file.
    crop_frame : bool, optional
        Flag to crop the frame (default is False)
        
    Returns
    -------
    cv2 OutputArray image
        The corrected image. 
    """  
    
    h, w = input_frame.shape[:2]  # get the height and width of the input frame   
    # apply undistortion
...
def init_video_writer(frame_width, frame_height, fps_cap, save_path, timestring, roi_name, encode_preset):
    """
    Initializes the video writer using subprocess.
    ...
    encode_preset : str
        How much processor time to spend encoding.
...
#    try:
#        print(sp.PIPE)
#        vid_writer = sp.check_call(ffmpeg_command, stdin=sp.PIPE)
#    except:
#        print("ERROR OCCURED - command exited with non_zero code")
#    video_writer = sp.Popen(ffmpeg_command, stdin=sp.PIPE, stdout=sp.PIPE, stderr=sp.PIPE)
    video_writer = sp.Popen(ffmpeg_command, stdin=sp.PIPE, shell=True)
...
def video_capture(child_conn_obj, data_q_obj, 
    ...
    Parameters
    ----------
    child_conn_obj : multiprocessing Connection object
        This is pretty much where the video data is coming from? 
    data_q_obj : multiprocessing Queue class
        The video data.
    calib_mtx : ndarray object
        The camera calibration data.
    calib_dist : ndarray object
        The camera calibration distances.
    fps_cap : int
        The max frames per second of the camera.
    roi_list : list
        List of arena ROI objects
    vid_params : list
        List of tuples containing the video height, width, and arena name.
    save_path : str
        Where to save the video.
    write_video : str
        How much processor time to spend encoding.
...
def choose_file(custom_text):
    """
    Hunts down the calibration file.
...
def load_cam_calib_file(filepath=None):
    ...
    filepath : str, optional
        Where the camera calibration file is located (default is None)
    """
    
    # Check if a filepath was specified, if not then ask for file
    if not filepath:
        filepath = choose_file("Please choose the camera calibration file you wish to load!")

    # If the file exists, check if it's a valid file then open if it is
    if os.path.exists(filepath):
        # Split into file name and its extension
        filename, file_extens = os.path.splitext(filepath)                    
        if file_extens == ".json":  # checks if it's a json
            with open(filepath, 'r') as data_file:  # read the json
                data = json.load(data_file)                
            if data:
                try:  # try to get calibration data
...
class InitArduino:
    ...
    Attributes
    ----------
    arduino : pySerial serial class
        The serial class for the Arduino. 
    is_on : boolean
        A flag indicating if the arduino is on (default is False)
    state : str
        Arduino state consists of 7 values - 1 for led_freq and 
        6 for the 6 arenas (LED_freq,LED_PW,SOL1,SOL2,SOL3,SOL4,SOL5,SOL6) 
        (default is '0.00,0.00,0,0,0,0,0,0'").
    arduino_readline : str
        When serial connection is made, arduino opto-blink script sends an 
        initial "OFF" signal. We'll just read the line and empty the serial 
        buffer.
    np_fromstring : numpy method
        For loop optimizations. This will allow us read the current state of
        the arduino.
    
    Methods
    -------
    update_state(new_state, roi_id)
    write(values)
    ...
    def __init__(self, port='COM8', baudrate=250000, timeout=0.02):
        """
        Parameters
        ----------
        port : str, optional
            The arduino name to connect to (default is "COM8").
        baudrate : int, optional
            Baud rate - can be one of the standard values (default is 250000).
        timeout : float, optional
            How long in seconds to wait before reading from the arduino 
            (default is 0.2 seconds).
        """
        
        #Initialize the arduino!
        #Doing it this way prevents the serial reset that occurs!
        self.arduino = serial.Serial()  # init. the serial class to read from arduino
        self.arduino.port = port  # set the port of the arduino
        self.arduino.baudrate = baudrate  # set baud rate for the arduino
        self.arduino.timeout = timeout  # set time out for the arduino
        self.arduino.setDTR(False)  # sets the state of the DTR line
        self.arduino.open()  # opens the port
        time.sleep(0.5)  # stops program for 0.5 seconds

        self.arduino.readline()  # read next line of the arduino
        self.is_on = False  # init. is_on
        
        self.state = '0.00,0.00,0,0,0,0,0,0'  # init. state
        
        #Loop optimizations
        self.arduino_readline = self.arduino.readline  # read next line
        self.np_fromstring = np.fromstring  # reference np method
    
#    def cooldown(func):
#        def wrapped(*args, **kwargs):
#            wrapped.last_call_time = time.clock()
...
#        
    def update_state(self, new_state, roi_id):
        """
        Updates the state of the arduino.
        
        Parameters
        ----------
        new_state : int?
            The new state of the arduino.
        roi_id : int?
            Which state index to update.
        """
        
        # First convert current state string of floats into numpy array
        prior_state = self.np_fromstring(self.state, dtype=float, sep=',')        
        state_indx = roi_id + 1  # add 1 because first item led_freq  
        
        # If new state is different from old state, update it.
        if prior_state[state_indx] - new_state != 0:  
            prior_state[state_indx] = new_state # update state
            ...
            new_state = ",".join(map(str, prior_state))  # convert state back to string
            self.write(new_state)  # write new state to arduino
        
    def write(self, values):
        """
        Sends values to the arduino then updates state with next line.
        ...
        values = "["+values+"]"
        self.arduino.write(values)  # write to the arduino
        self.state = self.arduino_readline()  # get next state of arduino
    ...
    def turn_on_stim(self, led_freq, led_dur):
        ...
        # send the stim data to the arduino
        self.arduino.write('[{freq},{dur}]'.format(freq=led_freq, dur=led_dur))
        self.state = self.arduino_readline()  # execute the stim data 
    ...
    def turn_off_stim(self):
        ...
        self.arduino.write('[0.00,0.00]')  # send stim off data to arduino
        self.state = self.arduino_readline()  # execute on the data
    ...
    def turn_off_solenoids(self):
        ...
        self.arduino.write('[0.00,0.00,0,0,0,0,0,0]')  # sends solenoid off data
        self.state = self.arduino_readline()  # executes solenoid off data
...
class Arena():
    ...
    *Optionally* an occupancy ROI instance
    ...
    ----------
    name : str
        Name of the particular arena (for example if you have 6 arenas, an
        arena may be named "Arena 1").
    write_video : bool
        Flag to export video (default is False).
    debug_mode : bool
        Flag for debug_mode (default is False).
    get_occupancy_roi : bool
        Flag for setting the occupancy ROI (default is False).
    sample_frame : cv2 OutputArray image object
        The sample frame from the camera. 
    fps_cap : int
        Max frames per second (default is 60).
    arena_id : int
        The arena number/label.
    arduino : InitArduino class
        The initialized arduino (default is None)
    use_arduino : bool
        Flag to use arduino (default is False)
    arduino_update_state : InitArduino update_state method
        Method to update the arduino for this particular arena.
    arena_roi : ?
        The ROI for this particular arena.
    bg_subtractor : BackgroundSubtractorKNN class
        Background subtractor for fly detection.
    bg_apply : method inherited from cv::BackgroundSubtractor
        Method to compute the foreground mask.
    bgs_learning_rate : float
        Learning rate for the KNN background subtractor (default is -1)
    fly_location_array : list
        Python list that has the fly's location (default is [])
    last_fly_locations : deque
        The last x and y coordinate of the fly.
    last_displacement : deque
        The last displacement of the fly.
    last_rewd_contour_status : deque
        The last rewd contour status.
    bgs_kernel : cv Mat class
        The structuring element for the particular fly arena.
    
    Methods
    -------
    init_video_writer(save_path, expt_start_date)
    close_video_writer()
    preprocess_frame(cropped_current_frame, time_ellapsed)
    track(preprocess_output)
    write_data_out(self, save_path, filename)
    """
    ...
    def __init__(self, arena_label, arena_id, input_frame, get_occupancy_roi=False, 
        ...
        ----------
        arena_label : str
            The name of this particular arena.
        arena_id : int
            The index or label of this particular arena.
        input_frame : cv2 OutputArray image object
            The sample frame from the camera.
        get_occupancy_roi : bool, optional
            Flag to get occupanct ROI (default is False).
        write_video : bool, optional
            Flag to save the video (default is False).
        debug_mode : bool, optional
            Flag to go into debug mode (default is False)
        fps_cap : int, optional
            The maximum frames per second of the camera or experiment 
            (default is 30)
        use_arduino : bool, optional
            Flag to use the arduino (default is False).
        arduino_obj : bool, optional
            The arduino class (default is None).
        """
    ...
    def init_video_writer(self, save_path, expt_start_date):
        """
        Initializes the video writer by calling init_video_writer.
    ...
    def preprocess_frame(self, cropped_current_frame, time_ellapsed):
        ...
        Parameters
        ----------
        cropped_current_frame : cv2 OutputArray image object
            The frame for this particular arena.
        time_ellapsed : str
    ...
    def track(self, preprocess_output):
        ...
        Parameters
        ----------
        preprocess_output : preprocessed frame object
            This is the return value of the preprocess_frame method. It 
            contains a dilate object, cropped frame object, and the time
            elapsed.
        """
        ...
        image, contours, hierarchy = cv2.findContours(dilate, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)  # find the fly
        ...
        if contours:            
            ...
            if len(contours) > 1:
                ...
                contour_area = contour_areas[max_indx]  # get area of fly
                contour = contours[max_indx]  # the fly is the biggest contour
            if len(contours) == 1:
                contour = contours[0]  # get fly from contours
                contour_area = cv2.contourArea(contour)  # get area of fly      
            if self.debug_mode:
                print "First contour in the given frame has area: {}".format(contour_area)            
                
            moments = cv2.moments(contour)            
            if moments['m00'] > 0:
                cx, cy = int(moments['m10']/moments['m00']), int(moments['m01']/moments['m00'])  # get coords of fly
                cv2.circle(cropped_copy, (cx, cy), 8, (255,255,255), thickness=1)                                
                self.last_fly_locations.append(np.array([cx,cy]))  # add to list of fly locations
                ...
                self.fly_location_array.append((time_ellapsed, cx, cy, self.last_rewd_contour_status[-1]))  # add time, coords, and reward status to fly location array 
        ...
        try:
            ...
            if self.last_displacement[0] <= 2.0:  # is this a mistake?
    ...
    def write_data_out(self, save_path, filename):
        ...
        ----------
        save_path : str
            Location you want the csv to go.
        filename : str
            Name of the csv.
        """
...
def preprocess_and_track(preproc_dict, track_dict, arena_dict_keys, cropped_frame_list, time_stamp):
    """
    Preprocess the each frame per arena then track the fly in the preprocessed
    frame.
    
    Parameters
    ----------
    preproc_dict : dict
        Dictonary of preprocessing methods for each arena.
    track_dict : dict
        Dictionary of tracking methods for each arena.
    arena_dict_keys : sorted view object of dict
        A sorted view object of the keys of each arena
    cropped_frame_list : list
        ?
    time_stamp : str
    ...
    preproc_output = [preproc_dict[key](cropped_frame_list[indx], time_stamp) for indx, key in enumerate(arena_dict_keys)]  # get preprocessed frame from each arena
    #tracking step. See: track()
    for indx, key in enumerate(arena_dict_keys):  # track the fly in each arena for each frame
...
def start_fly_tracking(expt_dur = 900, led_freq = 5, led_pw=5, fps_cap = 30, 
                       ...
                       default_save_dir = "D:\\Max Seppo"):
    """Starts the fly tracking experiment.
    
    Parameters
    ----------
    expt_dur : int, optional
        How long in seconds the experiment will run (default is 900 seconds 
        which is 15 minutes).
    led_freq : int, optional
        ?
    led_pw : int, optional
        ?
    fps_cap : int, optional
        Limit to how many FPS the camera can record (default is 30)
    debug_mode : bool, optional
        A flag to switch into debug mode (default is false)
    use_arduino : bool, optional
        A flag to use the arduino (default is False)
    write_video : bool, optional
        A flag to save video (default is True)
    write_csv : bool, optional
        A flag to save data to csv (default is False)
    num_arenas : int, optional
        How many arenas will be in the experiment (default is 4)
    define_occupancy_roi : bool, optional
        Flag to manually indicate ROI (default is True)
    cam_calib_file : str, optional
        Location of the camera calibration file (default is 
        "Camera_calibration_matrices.json")
    default_save_dir : str, optional 
        Location of where to save the output files (default is "D:\\Max Seppo")
    """
    ...
    if use_arduino:
        try:
            ...
            print("There was a problem connecting to the arduino! \
    ...
    # Get calibration matrix (np.array) from the calibration data
    calib_mtx = calibration_data["camera_matrix"]
    # Get the calibration distances (np.array) from the calibration data
    calib_dist = calibration_data["dist_coeff"]
      
    #Note if your laptop or computer has a built in webcam you may need to set
    #the videocapture element to 1 instead of 0
    temp_cam = cv2.VideoCapture(cv2.CAP_DSHOW + 0)  # init. the camera
    #We don't want the camera to try to autogain as it messes up the image
    ...
    for x in range(45):  # read in the 45th frame? or read in 45 frames?
    ...
    arena_dict = {'Arena {}'.format(arena):Arena('Arena {}'.format(arena), arena, temp_frame, 
                                                 ...
                                                 arduino_obj=arduino)
                  for arena in range(1, 1+num_arenas)}
    ...
    if write_video:
        #Initialize annotated tracking video writers for each arena
    ...
    if use_arduino:  # if using arduino, turn it off
        arduino.turn_off_stim()
        arduino.turn_off_solenoids()
        arduino.close()
        
    if write_csv:  # if saving data, export
        for key in arena_dict_keys:
            arena_dict[key].write_data_out(save_path, "{} - {}".format(expt_start_date, key))
            
    if write_video:  # if saving video, export
        for key in arena_dict_keys:
...
if __name__ == '__main__':    
    start_fly_tracking(expt_dur = 900,  # how long in seconds the experiment will be
                       write_video=True,  # if you want to save the experiment footage
                       define_occupancy_roi=True,  # if you want to define the ROI
                       use_arduino=True,  # if you're using an arduino
                       write_csv=True,  # if you want to export the data as a csv 
                       num_arenas=6)  # how many arenas are in the experiment
